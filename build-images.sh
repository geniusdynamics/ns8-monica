...
# Reuse existing nodebuilder-monica container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-monica; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-monica -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/library/redis:7.2.3-bookworm docker.io/postgres:15.5-alpine3.19 docker.io/getmeili/meilisearch:latest ghcr.io/monicahq/monica-next:main docker.io/mariadb:10.11.5" \
    "${container}"
# Commit the image
